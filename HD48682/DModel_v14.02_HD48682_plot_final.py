...
#                Flam = 32*pi**3*dNR*(s*10**-6)**3*(dpc*pc)**-2*QabsG(s,wr).flatten()*BBfdm*(rbins_bm[i]/hd_bs) #Thermal Emission
                Flam = 8*pi**2*dNR*(s*10**-6)**3*(dpc*pc)**-2*QabsG(s,wr).flatten()*BBfdm*(rbins_bm[i]/hd_bs) #Thermal
...
#                Flamb = 32*pi**3*dNR*(s*10**-6)**3*(dpc*pc)**-2*QabsG(s,wr).flatten()*BBf*(rbins_bm[i]/hd_bs)  #Thermal Emission
                Flamb = 8*pi**2*dNR*(s*10**-6)**3*(dpc*pc)**-2*QabsG(s,wr).flatten()*BBf*(rbins_bm[i]/hd_bs)  #Thermal
...
#            dNbr = np.trim_zeros(dNb_apert)  #remove zeroes from array
            ...
            #Thermal Emission
#            Flam = BBfns_sum*32*pi**3*(s*10**-6)**3*(dpc*pc)**-2/hd_bs
...
def lnprior(theta):
    ...
    if 0.6 < sm < 10.0 and dfrac < 0.1 and 3.0 < q < 4.0 and 50 < rm < 150 and 0.1 < rw < 60:
        return 0.0
    else:
        return -np.inf
...
lam = [3.4,    4.6,    9.0,    12.0,   18, 22.0,  30, 32, 34, 70.0,  100.0,  160.0,  250.0,  350.0,  850.0]
flx = [8.6659, 4.5114, 1.3534, 0.7610, 0.4625, 0.2444, 0.148, 0.142, 0.136, 0.290,  0.275,  0.177,  0.09,   0.025,  0.0039]
unc = [2.7266, 1.1322, 0.0451, 0.0113, 0.0631, 0.005,  0.013, 0.017, 0.020, 0.038,  0.007,  0.024,  0.015,  0.008,  0.0008]
...
##Astrosilicate
#sm = 6.061 #4.44 +1.29 -1.10
#smax = 3000
#dfrac = 0.04032 #0.00367 +0.00206 -0.00136
#rm = 88.301 #86.67 +45.97 -43.02
#rw = 78.493 #33.12 +17.93 -20.62
#qv = 3.884 #3.83 +0.11 -0.20
#rin = 10
#rout = 220

#Dirty Ice
sm = 7.923 #4.44 +1.29 -1.10
smax = 3000
dfrac = 0.02292 #0.00367 +0.00206 -0.00136
rm = 81.003 #86.67 +45.97 -43.02
rw = 70.853 #33.12 +17.93 -20.62
qv = 3.608 #3.83 +0.11 -0.20
rin = 10
rout = 220
...
c2 = Chi2(lam,flx,unc,wr ,SED_total2)
#chi2.append(c2)
print(f'Chi2: {c2}')
...
#            bgf_b.append(val)
    ...
    Fluxvalsg_nb = df_fluxsm.loc[df_fluxsm['Wavelength (um)'] == Wavelength]

    bgs_nb_s = [] #gs: grain grain  ('b' for beta?)
    bgd_nb_s = [] #gd: grain distance
    bgf_nb_s = [] #gf: grain flux

    bgs_nb_b = [] #gs: grain grain  ('b' for beta?)
    bgd_nb_b = [] #gd: grain distance
    bgf_nb_b = [] #gf: grain flux
    
    for column in Fluxvalsg.columns[2:-1]:  #Extract all flux values except the 'Sum'(last) column
        val = Fluxvalsg.iloc[0][column]              #pulls all flux values at specific wavelength
        bvalues = re.findall('\d*\.?\d+',column)    #finds all numbers in column name and places in a 2-element array of strings
        bgs_nb.append(float(bvalues[0]))               #append grain size
        bgd_nb.append(float(bvalues[1]))               #append grain distance
        bgf_nb.append(val)                             #append Flux value
        if float(bvalues[0]) <= 10:
            bgs_nb_s.append(float(bvalues[0]))               #append grain size
            bgd_nb_s.append(float(bvalues[1]))               #append grain distance
            bgf_nb_s.append(val)                               #append Flux value
        else:
            bgs_nb_b.append(float(bvalues[0]))               #append grain size
            bgd_nb_b.append(float(bvalues[1]))               #append grain distance
            bgf_nb_b.append(val)
   
    df_fldis_nb_s = pd.DataFrame({'Distance': bgd_nb_s, 'Flux': bgf_nb})
    df_fldis_nb_s = df_fldis_nb.groupby('Distance', as_index=False).sum()
    ax.plot(df_fldis_nb_s['Distance'],df_fldis_nb_s['Flux'], label = 'Non-beta')
    
    
    df_fldis_s = pd.DataFrame({'Distance': bgd_s, 'Flux': bgf_s})
    df_fldis_s = df_fldis_s.groupby('Distance', as_index=False).sum()
#    ax.plot(df_fldis_s['Distance'],3*df_fldis_s['Flux']/np.max(df_fldis_s['Flux']), label = 's <= 10')
    ax.plot(df_fldis_s['Distance'],df_fldis_s['Flux'], label = 'Beta')
...
#    ax.plot(df_fldis_b['Distance'],df_fldis_b['Flux'], label = 's > 10')
#
#    ax.set_xscale('log')
    ax.set_xlim([0,300])
    ax.set_yscale('log')
#    ax.xaxis.set_major_formatter(formatter)
    ax.yaxis.set_major_formatter(formatter)
    ax.set_xlabel('Stellar Distance [au]')
    ax.set_ylabel('Arbritrary Flux Offset')
    ax.legend()

#    figname = object+'_'+composition+'_Radial_profile_grainregimes_bnb_'+str(round(Wavelength,2))+'.eps'
    figname = object+'_'+composition+'_Radial_profile_smallgrains_bnb_'+str(round(Wavelength,2))+'.eps'
    plt.savefig(figname)
