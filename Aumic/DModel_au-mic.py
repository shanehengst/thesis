...
#    if 0.01 < sm < 20 and 0.001 < dfrac < 1.0 and 3.0 < q < 4.0 and 20 < rm < 47 and 1 < rw < 40:
#        return 0.0
#    else:
#        return -np.inf
#
...
def lnlike(theta,phot_wav,phot_flux,phot_unc):
    #sm,dfrac,qv,rm,rw = theta
    sm,dfrac,qv = theta
    func_flux = DustyMM(sm,smax_r,dfrac,qv,rmean,rwidth,rin,rout)
    func_wf = interpolate.interp1d(wr,func_flux)
    flam = func_wf(phot_wav)
#    #print(flam)
    c2 = 0
...
#mir_spec2 = []
#
#for i in range(len(mir_2spe)):
#    if mir_2spe[i] == 0:
#        mir_wave1.append(mir_wave[i])
#        mir_spec1.append(mir_spec[i])
#    if mir_2spe[i] == 1:
#        mir_wave2.append(mir_wave[i])
#        mir_spec2.append(mir_spec[i])
#
#
#
#
...
nwalkers = 30
niter = 200
#initial = np.array([3.34,0.06,3.26,30,20]) #Variables to be tested: sm,dfrac,qv,rm,rw #astro-silicate
##initial = np.array([1.5,0.25,3.62,120,25]) #Variables to be tested: sm,dfrac,qv,rm,rw #dirty ice
#
#ndim = len(initial)
...
initial = np.array([2,0.15  ,3.5]) #Variables to be tested: sm,dfrac,qv
ndim = len(initial)
p0 = [np.array(initial) + np.array([random.uniform(-4.3,2.0),random.uniform(-0.03,0.03),random.uniform(-0.5,0.5)],) for i in range(nwalkers)]
