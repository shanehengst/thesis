...
f_mirs_u = interpolate.interp1d(mir_wave, mir_unce, kind = 'linear',fill_value = 'extrapolate')
...
mir_lam = [3.350,  4.6,    5.800,  8,      9.0000, 11.60,  18.00,  22.10,  24] #c12: Cutri & et al. 2012,W10: Wright et al. 2010, C05 Chen et al. 2005, I10, Ishihara 2010, P09. Plavchan 2009
...
#HSA: Matthews Herschel in prep at the time
fir_lam = [70.00,160.0]
fir_flx = [0.231,0.243]
fir_unc = [0.016,0.017]

#L04: Liu et al 2004,W12 Wilner 2012
smm_lam = [250.0,350.00,450.0,500.00, 850.00,1300.0] #
smm_flx = [0.134,0.0844,0.085,0.0476,0.0144,0.0085]
smm_unc = [0.008,0.0054,0.042,0.0038,0.0018,0.0020]

#Stellar Photometric values for comparison
#
##lam = [3.350,4.600,5.800,8.00,9.0000,11.60,12.00,18.00,22.10,70.00,160.0,250.0,350.00,450.0,500.00,850.00,1300.0]
###flx = [4.820,4.260,2.200,1.27,0.9120,0.543,0.517,0.246,0.183,0.231,0.243,0.134,0.0844,0.085,0.0476,0.0144,0.0085]
###unc = [0.199,0.199,0.055,0.02,0.0022,0.060,0.030,0.036,0.021,0.016,0.017,0.008,0.0054,0.042,0.0038,0.0018,0.0020]
##lam = [11.60,12.00,18.00,22.10,70.00,160.0,250.0,350.00,450.0,500.00,850.00,1300.0]
##flx = [0.543,0.517,0.246,0.183,0.231,0.243,0.134,0.0844,0.085,0.0476,0.0144,0.0085]
##unc = [0.060,0.030,0.036,0.021,0.016,0.017,0.008,0.0054,0.042,0.0038,0.0018,0.0020]
#
...
Ms = 0.6        #Mass of Star [Solar Masses]
Rs = 0.75          #Radius of Star [Solar Radius]
Ts = 3500       #star temperature [K]
dpc = 9.71       #distance to star system [pc]
...
#f_Ks = fun_spek(np.log10(sc_lam))
#f_Ks_nu = (10**f_Ks)*((sc_lam*10**-6)**2)/(c)

flux_nu = sc_flx*flux_nu/np.max(flux_nu)
...
mir_lam = [3.350,4.600,5.800,8.00,9.0000,11.60,12.00,18.00,22.10]
mir_flx = [4.820,4.260,2.200,1.27,0.9120,0.543,0.517,0.246,0.183]
mir_unc = [0.199,0.199,0.055,0.02,0.0022,0.060,0.030,0.036,0.021]


fir_lam = [70.00,160.0,250.0,350.00,450.0,500.00,850.00,1300.0]
fir_flx = [0.231,0.243,0.134,0.0844,0.085,0.0476,0.0144,0.0085]
fir_unc = [0.016,0.017,0.008,0.0054,0.042,0.0038,0.0018,0.0020]
...
ax.plot(wr,flux_nu, 'k:', label = 'Star:'+object, zorder = 1)
ax.plot(wr,SED_disc, 'b-.', label = 'Disc:dirty ice', zorder = 2)
ax.plot(wr,SED_total, 'k', label = 'Star+Disc', zorder = 3)
#ax.plot(wr, SED_total, 'c-', label = 'Star+Disc+Fast')
##ax.plot(wr,SED_total2, 'k-', label = 'Star+Disc+Slow')
#ax.plot(wr,SED_disc2, 'c-.', label = 'Dics2')
#ax.plot(wr,SED_total2, 'c-', label = 'Star+Disc2')
ax.plot(mir_wave,mir_spec,'b',label = 'IRS Spectrum')
#ax.errorbar(mir_wave,mir_spec,yerr=mir_unce,fmt='.',color='blue',ecolor='blue', label = 'IRS Spectrum')
...
ax.errorbar(mir_lam,mir_flx,yerr=mir_unc,fmt='o',mec='skyblue',mfc='skyblue',ecolor='black',capsize=4.,capthick=1, label = 'Mid-IR',zorder=5)
ax.errorbar(syn_lam,syn_flx,yerr=syn_unc,fmt='o',mec='cyan',mfc='cyan',ecolor='black',capsize=4.,capthick=1, label = 'Synth-Phot',zorder=6)
ax.errorbar(fir_lam,fir_flx,yerr=fir_unc,fmt='o',mec='red',mfc='red',ecolor='black',capsize=4.,capthick=1, label = 'Far-IR',zorder=7)
ax.errorbar(smm_lam,smm_flx,yerr=smm_unc,fmt='o',mec='y',mfc='y',ecolor='black',capsize=4.,capthick=1, label = 'Sub-mm',zorder=8)

#ax.errorbar(lam,flx,yerr=unc,fmt='o',mec='red',mfc='red',ecolor='black',capsize=4.,capthick=1, label = 'Phot+ColdOnly', zorder = 4)
...
plt.savefig('SED_Star+disc_para_models_au_mic_'+composition+'.eps')

plt.clf()
fig, ax = plt.subplots(nrows = 1, ncols = 1)
ax.plot(rbins_bm,df_rbins_dNb['Sum'],'b-.',label = 'Beta disc')
ax.plot(rbins_bm,df_rbins_dN['Sum'], 'k', label = 'non-Beta disc')
ax.set_xlim([1,200])
...
#ax.set_yscale('log')
ax.legend()
plt.savefig('Numberdensity_distance_'+composition+'.eps')

#compare fast and slow
SED_fs = SED_disc - SED_disc2
print(SED_fs)
plt.clf()
fig, ax = plt.subplots(nrows = 1, ncols = 1)
ax.plot(wr, SED_fs)
ax.set_xlabel('Wavelength [$\mu$m]')
ax.set_ylabel('Residual Flux Density [Jy]')
ax.set_xlim([0.3, 3000])
ax.set_ylim([10**-3, 20])
ax.set_xscale('log')
ax.set_yscale('log')
ax.xaxis.set_major_formatter(formatter)
ax.yaxis.set_major_formatter(formatter)
plt.savefig('Flux_Fast_Slow_'+composition+'.eps')
