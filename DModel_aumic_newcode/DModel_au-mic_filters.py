...
#-----------------------------------------------------------------#
###Function: probability function for orbit of grain
...
def DustyMM_ext(smin,smax,dfrac,q,rm,rw,rin,rout):
    ##Planestimal Belt Characteristics##
    print(f'Min. Grain Size: {smin}, dfrac: {dfrac}, q: {q}, rmean: {rm}, rsigma: {rw}')
    ##Total Grain Mass##
    DiscMass = dfrac*Me*10**3 #Total grain mass as a fraction of Earth Mass (converted to grams) because denisty(rho) is given in g/cm^3
...
def DustyMM_ext1(smin,smax,dfrac,q,rm,rw,rin,rout,filter):
    ##Planestimal Belt Characteristics##
    print(f'Min. Grain Size: {smin}, dfrac: {dfrac}, q: {q}, rmean: {rm}, rsigma: {rw}')
    ##Total Grain Mass##
    DiscMass = dfrac*Me*10**3 #Total grain mass as a fraction of Earth Mass (converted to grams) because denisty(rho) is given in g/cm^3
...
def radpressure(Ls,Ms,Rs,Ts,rho,composition):
    ...
    flux_sar = 10**(f_10spek)
    BB_star = Blam(Ts,wrange*10**-6)
    BStar = np.max(BB_star)*(flux_sar/np.max(flux_sar))
...
#    if 0.01 < sm < 15.0 and 0.001 < dfrac < 0.1 and 3.0 < q < 4.0 and 125 < rm < 145 and 5 < rw < 15:
    if 0.01 < sm < 30.0 and 0.001 < dfrac < 0.5 and 3.0 < q < 4.0: #
        return 0.0
...
lam = [0.43,    0.55,   3.350,  5.800,  9.0000, 11.60,  18.00,  22.10,  24,     35,         70.00,160.0,250.0,350.00,450.0,500.00,850.00,1300.0]
flx = [0.278,   0.985,  4.820,  2.200,  0.9120, 0.543,  0.246,  0.183,  0.158,  f_syn_35,   0.231,0.243,0.134,0.0844,0.085,0.0476,0.0144,0.0085]
unc = [0.104,   0.185,  0.199,  0.055,  0.0022, 0.060,  0.036,  0.021,  0.03,   f_syn_35_u, 0.016,0.017,0.008,0.0054,0.042,0.0038,0.0018,0.0020]
#Import Photometric values with warm component subtract
#Phot_aumic = np.loadtxt('df_aumic_cold.txt', dtype=[('wavelength', float),('flux', float),('uncertainty', float) ])
#lam = Phot_aumic["wavelength"]
#flx = Phot_aumic["flux"]
#unc = Phot_aumic["uncertainty"]
#
#print(lam)
#print(flx)

onr_lam = [0.43, 0.55, 1.235,1.662,2.159,3.350,4.600,5.800,  8.00,   9.0000, 11.60, 12.00]
onr_flx = [0.278,0.985,10.7,    12, 10.3,  4.820,4.260,2.200,1.27,   0.9120, 0.543, 0.517]
onr_unc = [0.104,0.185,0.06,    0.06, 0.06, 0.199,0.199,0.055,0.02,   0.0022, 0.060, 0.030]
...
flux_nu = sc_flx*flux_nu/np.max(flux_nu)  #scaled flux
flux_sa = flux_nu
...
rin = 5
rout = 50
#rmean = 36 #2012 Wilner et al.
#rwidth = 10 #2012 Wilner et al.
rmean = 26.7 #2019 Daley et al. mean distance between stellocentric distance of ansae
rwidth = 3#  approx. sigma width


#Tpr_range = [1000, 22e6,1e9]
Tpr_range = [22e6]
tau = 4.5e-4 #approx. value determined by Schupller2007
...
#Dirty Ice
sm_mcmc = [1.564,1.69,-0.969]
dfrac_mcmc = [0.06664,0.0077,-0.00489]
q_mcmc = [3.206,0.07,-0.051]

#astro-silicate
#sm_mcmc = [0.755,0.66,-0.433]
#dfrac_mcmc = [0.15508,0.03422,-0.002171]
#q_mcmc = [3.368,0.05,-0.043]
...
#
#    [SED_total,SED_disc,Mperb] = DustyMM(0.6,3000,0.15,3.36,rmean,rwidth,rin,rout)
#
...
#    print(zed)
    ...
    initial = np.array([1,0.15,3.5]) #Variables to be tested: sm,dfrac,qv
    ndim = len(initial)
    p0 = [np.array(initial) + np.array([random.uniform(-0.5,10.0),random.uniform(-0.05,0.05),random.uniform(-0.5,0.5)],) for i in range(nwalkers)]
...
#    sm_mcmc, dfrac_mcmc, q_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), zip(*np.percentile(samples, [16,50,84],axis=0)))
    ...
    direcsav = '/'+object+'_smin'+str(round(sm_mcmc[0],2))+'_rm'+str(rmean)+'_dfrac'+str(round(dfrac_mcmc[0],2))+'_q'+str(round(q_mcmc[0],2))+'_rw'+str(round(rwidth,2))+'_rin'+str(rin)+'_rout'+str(rout)+'_ch2'+str(round(c2,2))+'_tpr'+f'{Tpr:.2E}'
...
#    fir_unc = [0.063,0.096,0.0955,0.1512e-3]
    ...
    mir_lam = [3.350,  4.6,    5.800,  8,      9.0000, 11.60,  18.00,  22.10,  24]
    mir_flx = [4.820,  4.260,  2.200,  1.270,  0.9120, 0.543,  0.246,  0.183,  0.158]
    mir_unc = [0.199,  0.199,  0.055,  0.0020, 0.0022, 0.060,  0.036,  0.021,  0.03]

    mir_slam = [35]
    mir_sflx = [f_syn_35]
    mir_sunc = [f_syn_35_u]

    fir_lam = [70.00,160.02,250.0,350.00,450.0]
    fir_flx = [0.231,0.243,0.134,0.0844,0.085]
    fir_unc = [0.016,0.017,0.008,0.0054,0.042]

    smm_lam = [500.00, 850.00,1300.0]
    smm_flx = [0.0476,0.0144,0.0085]
    smm_unc = [0.0038,0.0018,0.0020]
    ...
    ax.plot(mir_wave,mir_spec,'b',label = 'IRS Spectrum',zorder=4)
#    ax.errorbar(irs_spec['wavelength'],irs_spec['flux'],yerr=irs_spec['error'],fmt='.',color='blue',ecolor='blue', label = 'IRS Spectrum',zorder = 4)
    ...
    ax.errorbar(fir_lam,fir_flx,yerr=fir_unc,fmt='o',mec='red',mfc='red',ecolor='black',capsize=4.,capthick=1, label = 'Far-IR',zorder=8)
    ax.errorbar(smm_lam,smm_flx,yerr=smm_unc,fmt='o',mec='y',mfc='y',ecolor='black',capsize=4.,capthick=1, label = 'Sub-mm/mm',zorder=9)
    ...
    for Wavelength in WRange:
        ...
        ax.plot(rbins_bm,f_flux(rbins_bm)/np.max(f_flux(rbins_bm)),str(WColour[Ci])+'-',label = str(round(Wavelength,3))+'$\mu$m')
    #    ax.plot(rbins_bm,f_flux(rbins_bm),str(WColour[Ci])+'-',label = str(round(Wavelength,3))+'mu m')
        Ci = Ci + 1
    ...
    ax.xaxis.set_major_formatter(formatter)
    #ax.yaxis.set_major_formatter(formatter)
    plt.savefig('Flux_distance_'+object+'_'+composition+'.pdf')
...
#    MIRI_10 = DustyMM_fdist(sm_mcmc[0],smax_r,dfrac_mcmc[0],q_mcmc[0],rmean,rwidth,rin,rout,fMIRI_F100wr)
    MIRI_10 = DustyMM_ext1(sm_mcmc[0],smax_r,dfrac_mcmc[0],q_mcmc[0],rmean,rwidth,rin,rout,fMIRI_F100wr)
...
#    MIRI_21 = DustyMM_fdist(sm_mcmc[0],smax_r,dfrac_mcmc[0],q_mcmc[0],rmean,rwidth,rin,rout,fMIRI_F210wr)
    MIRI_21 = DustyMM_ext1(sm_mcmc[0],smax_r,dfrac_mcmc[0],q_mcmc[0],rmean,rwidth,rin,rout,fMIRI_F210wr)
...
#    PACS_70 = DustyMM_fdist(sm_mcmc[0],smax_r,dfrac_mcmc[0],q_mcmc[0],rmean,rwidth,rin,rout,fFBlue70wr)
    PACS_70 = DustyMM_ext1(sm_mcmc[0],smax_r,dfrac_mcmc[0],q_mcmc[0],rmean,rwidth,rin,rout,fFBlue70wr)
...
#    PACS_100 = DustyMM_fdist(sm_mcmc[0],smax_r,dfrac_mcmc[0],q_mcmc[0],rmean,rwidth,rin,rout,fFGreen100wr)
    PACS_100 = DustyMM_ext1(sm_mcmc[0],smax_r,dfrac_mcmc[0],q_mcmc[0],rmean,rwidth,rin,rout,fFGreen100wr)
    
    print('Determine Filter Values for Herschel/PACS 70 Filter band...')
#    PACS_160 = DustyMM_fdist(sm_mcmc[0],smax_r,dfrac_mcmc[0],q_mcmc[0],rmean,rwidth,rin,rout,fFRed160wr)
    PACS_160 = DustyMM_ext1(sm_mcmc[0],smax_r,dfrac_mcmc[0],q_mcmc[0],rmean,rwidth,rin,rout,fFRed160wr)
