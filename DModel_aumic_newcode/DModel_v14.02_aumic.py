...
##    SED_totalnb = np.add(flux_nb,flux_nu)
...
#    if 0.01 < sm < 15.0 and 0.001 < dfrac < 0.1 and 3.0 < q < 4.0 and 65 < rm < 110 and 0.1 < rw < 130:
    if 0.01 < sm < 15.0 and 0.001 < dfrac < 0.3 and 3.0 < q < 4.0: #noting blowout for dirty ice is 2.69 micron for HD 105211 / astrosilicate: blowout 1.601 0.044 -> 0.711
        return 0.0
...
lam = [11.60,   12.00,  18.00,  22.10,  35,         70.00,  160.0,  250.0,  350.00, 450.0,  500.00, 850.00, 1300.0]
flx = [0.543,   0.517,  0.246,  0.183,  f_syn_35,   0.231,  0.228,  0.134,  0.0844, 0.0492,  0.0476, 0.0140, 0.0085]
unc = [0.060,   0.030,  0.036,  0.021,  f_syn_35_u, 0.007,  0.015,  0.008,  0.0054, 0.0085,  0.0038, 0.0015, 0.0020]
...
#f_Ks = 4.82
#
#fluxstar = ascii.read('ckp00_3750.ascii')
#fluxstar = np.loadtxt('ckp00_3750.ascii', dtype=[('wavelength', float), ('flux', float)])
...
nwalkers = 30
niter = 200
initial = np.array([0.2,0.125,3.5]) #Variables to be tested: sm,dfrac,qv
ndim = len(initial)
p0 = [np.array(initial) + np.array([random.uniform(-0.19,14),random.uniform(-0.124,0.125),random.uniform(-0.5,0.5)],) for i in range(nwalkers)]
...
cornerfig = object+'_Emcee_corner_nwalkers'+str(nwalkers)+'_niter'+str(niter)+composition+'.eps'
