...
def lnprior(theta):
    ...
    if 1.9 < sm < 20 and 0.001 < dfrac < 1.0 and 3.0 < q < 4.0 and 15 < rm < 150 and 10 < rw < 200:
        return 0.0
    else:
        return -np.inf
...
nwalkers = 30
niter = 200
initial = np.array([6.2,0.29,3.3,23,24]) #Variables to be tested: sm,dfrac,qv,rm,rw
ndim = len(initial)
...
p0 = [np.array(initial) + np.array([random.uniform(-1,1),random.uniform(-0.05,0.05),random.uniform(-0.01,0.01),random.uniform(-3,3),random.uniform(-10,10)],) for i in range(nwalkers)]
...
cornerfig = object+composition+'_Emcee_corner_nwalkers_'+str(nwalkers)+'_niter'+str(niter)+'.eps'
...
DModel_emcee_values = 'DModel_emcee_values_'+composition+'_'+str(nwalkers)+'_'+str(niter)+'.txt'
