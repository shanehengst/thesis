...
#    print(zed)
    ...
    for rp in rp_space:
        ...
        P = af**(3/2)               #Period of grain for one orbit [years]
    ...
    for s in s_gs:
        ...
        if be > 0.5:
            ...
            if gr_blowout[-1] != s:
                gr_blowout.append(s)
        if be < 0:
            print(f'something is screwed (be < 0), beta = {be}, Ls = {Ls}, Qpr = {Qpr_s(s)}')
            print(zed)
...
#                denb = prob_grain_rp_p(be,r_belt[i],Tpr) #beta, given radius, Time (PR)
#
...
rmean = 133.9
rwidth = 23.46/2.355
Tpr = 1e9  #Fixed for now but may need to vary
...
#df_rbins_s = df_rbins_s.transpose()  #transpose the dataframe diagonally
#
...
#
#plt.pcolor(rbins_bm, s_gs, df_rbins_s, cmap = my_cmap,norm=colors.LogNorm(),linewidth=0,rasterized=True) #,
...
#
#
#title = 'Size_spatdist_timestep_'+str(Tpr)+'_q_'+str(q)+'.pdf'
#plt.savefig(title)
...
#    tau = emcee.autocorr.integrated_time(samples, tol=0)
    tau = samples.get_autorcorr_time()
...
converge, nt = check_convergence(reader)
...
##print("flat log prior shape: {0}".format(log_prior_samples.shape))
#
#new_backend = emcee.backends.HDFBackend(fname)
#print("Initial size: {0}".format(new_backend.iteration))
#new_sampler = emcee.EnsembleSampler(
#    nwalkers, ndim, lnprob, backend=new_backend, args=data
#)
...
direcsav = '/'+object+'_smin'+str(round(sm_mcmc[0],2))+'_rm'+str(rmean)+'_dfrac'+str(dfrac_mcmc[0])+'_q'+str(q_mcmc[0])+'_rw'+str(rwidth)+'_rin'+str(rin)+'_rout'+str(rout)+'_ch2'+str(c2)+'_tpr'+str(Tpr   )
