...
#Tprs = f'{Tpr:.2E}'
#Tpr_range = [1000,1.30e9,3.99e9]
Tpr_range = [3.99e9]
tau = 7.4e-5 #Or, use the Fracitonal Luminosity value calculated in my paper!!!!
#Face-on optical depth of standard debri disc. Adopt an upper bound of tau = 8 x 10^-3 Hughes et el. 2018
...
df_rb_s = pd.DataFrame({'Distance (au)': rbins_bm})


#Dirty Ice
#sm_mcmc = [1.564,1.69,-0.969]
#dfrac_mcmc = [0.0664,0.0770,-0.00489]
#q_mcmc = [3.206,0.07,-0.051]

#astro-silicate
sm_mcmc = [5.902,0.87,-0.638]
dfrac_mcmc = [0.07701,0.0521,-0.00545]
q_mcmc = [3.801,0.04,-0.038]
...
for Tpr in Tpr_range:
    ...
    #        print(f'{be}, {s}')
        ...
        df_rb_s[s_col] = den*(be/0.25)**3.5 #to be compared with later
...
#    sm_mcmc, dfrac_mcmc, q_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), zip(*np.percentile(samples, [16,50,84],axis=0)))
    ...
    direcsav = '/'+object+'_smin'+str(round(sm_mcmc[0],2))+'_rm'+str(rmean)+'_dfrac'+str(round(dfrac_mcmc[0],2))+'_q'+str(round(q_mcmc[0],2))+'_rw'+str(round(rwidth,2))+'_rin'+str(rin)+'_rout'+str(rout)+'_ch2_'+str(round(c2,2))+'_tpr'+f'{Tpr:.2E}'
...
#    #
    ...
    df_s_den = df_rbins_smin.sum()  #number density as a function of grain size
    df_s_den_q = df_rb_s.sum()
    
    
    plt.clf()
    fig, ax = plt.subplots(nrows = 1, ncols = 1)
    ax.plot(s_gs,df_s_den, 'k-', label = 'All dust grains')
#    ax.plot(s_gs,df_s_den_q, 'r:', label = 'Dust grains with q = 0')
...
#    ax.set_xlim([0,300])
    ax.set_ylabel('Relative Number of Grains [-]')
...
#    ax.set_xscale('log')
    ...
    ax.yaxis.set_major_formatter(formatter)
    #ax.legend()
    plt.savefig('Num_of_grains_by_size_'+object+'_'+composition+'.pdf')
...
#    my_cmap = copy.copy(matplotlib.cm.get_cmap('Reds')) # copy the default cmap
