...
#    if 0.01 < sm < 15.0 and 0.001 < dfrac < 0.1 and 3.0 < q < 4.0 and 65 < rm < 110 and 0.1 < rw < 130:
    if 0.01 < sm < 15.0 and 0.001 < dfrac < 0.1 and 3.0 < q < 4.0: #noting blowout for dirty ice is 1.1 micron for HD 48682 / astrosilicate: blowout 0.044 -> 0.711
        return 0.0
    else:
        return -np.inf
...
initial = np.array([2,0.05,3.5]) #Variables to be tested: sm,dfrac,qv
ndim = len(initial)
...
p0 = [np.array(initial) + np.array([random.uniform(-1.9,2.0),random.uniform(-0.03,0.03),random.uniform(-0.5,0.5)],) for i in range(nwalkers)]
