...
#    if 0.01 < sm < 15.0 and 0.001 < dfrac < 0.1 and 3.0 < q < 4.0 and 65 < rm < 110 and 0.1 < rw < 130:
    if 1.1 < sm < 15.0 and 0.001 < dfrac < 0.1 and 3.0 < q < 4.0: #noting blowout for dirty ice is 1.1 micron for HD 48682 / astrosilicate: blowout 0.044 -> 0.711
        return 0.0
    else:
        return -np.inf
...
rout = 220
rmean = 89
rwidth = 36.5 #sigma width


##Emcee Inputs##
nwalkers = 30 #50
niter = 200  #200
#initial = np.array([5.18,0.027,3.58,88,64]) #Variables to be tested: sm,dfrac,qv,rm,rw
...
#p0 = [np.array(initial) + np.array([random.uniform(-4.99,5.0),random.uniform(-0.01,0.01),random.uniform(-0.2,0.2),random.uniform(-10,10),random.uniform(-15,15)],) for i in range(nwalkers)]
p0 = [np.array(initial) + np.array([random.uniform(-3.8,4.0),random.uniform(-0.03,0.03),random.uniform(-0.5,0.5)],) for i in range(nwalkers)]
...
#
###Values for Astro-Silicate previously solved
##sm_mcmc = [4.805,0.85,0.621]
##dfrac_mcmc = [0.0445,0.0031,0.00364]
##q_mcmc = [3.835,0.08,0.074]
#
###New Directory folder
#direcsav = '/'+object+'_smin'+str(round(sm_mcmc[0],2))+'_rm'+str(rmean)+'_dfrac'+str(dfrac_mcmc[0])+'_q'+str(q_mcmc[0])+'_rw'+str(rwidth)+'_rin'+str(rin)+'_rout'+str(rout)
...
[SED_total,SED_disc,Mperb] = DustyMM(sm_mcmc,smax_r,dfrac_mcmc,q_mcmc,rmean,rwidth,rin,rout)
...
[df_rbins_p, df_rbins_pb, df_rbins_dN, df_rbins_dNb, df_rbins_dM,df_rbins_dMb, df_fluxsm, df_fluxsmb, y_values_nb, y_values_b, grainsizes,gr_blowout,MPerb,SED_total2,SED_total_nb2,SED_disc2,SED_disc_nb2] = BetaGrains(sm_mcmc,smax_r,dfrac_mcmc,q_mcmc,rmean,rwidth,rin,rout,rho,100,hd_bs,Tg_sr,Qpr_s,Qsca_sw,Qabs_sw,wr)

#                    print(np.max(SED_disc2))

#Fraction of disc mass
#dfracf = dfrac_mcmc[0]*Mperb/100
dfracf = dfrac_mcmc*Mperb/100
