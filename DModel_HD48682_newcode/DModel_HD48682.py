...
#            print(zed)
            ...
            BBfns_mod3 = 10**26*BBfns_mod2*((np.vstack(wr)*10**-6)**2)/(c)
            
            print(BBfns_mod3.shape)
            print(zed)
            
#            plt.clf()
#            plt.plot(wr,BBfns_mod3)
#            plt.yscale('log')
#            plt.xscale('log')
#            plt.ylim([10**-15,10**-5])
#            plt.xlim([10,500])
#
##            plt.imshow(BBfns_mod3)
#            plt.savefig('BBfns_mod3.pdf')
#
#
            BBfns_filter = np.multiply(BBfns_mod3,np.vstack(Flux_Filter))
#            plt.clf()
#            plt.imshow(BBfns_filter)
#
#            plt.plot(wr,BBfns_filter)
#            plt.yscale('log')
#            plt.xscale('log')
#            plt.ylim([10**-15,10**-5])
#            plt.xlim([10,500])
#
#            plt.savefig('BBfns_filter.pdf')
            
#            plt.show()
            
            
            #multiply blackbody fns by corresponding number density * radial distance
            

            
#            BBfns = np.multiply(np.multiply(BBfns,Qabs_sw(s,wr)),np.multiply(dNbr[::-1],gloc[::-1]))    #multiply blackbody fns by corresponding number density * radial distance
                            #BBfns = np.multiply(BBfns,dNbr)    #multiply blackbody fns by corresponding number density * radial distance
            Fnu_r = np.sum(BBfns_filter, axis = 0)     #sum all BB all flux values as a function of distance together
#            plt.clf()
#            plt.plot(rbins_bm,Fnu_r)
#            plt.xlim([0,300])
#
#            plt.show()
#            print(zed)
...
##    SED_totalnb = np.add(flux_nb,flux_sa)
    ...
    plt.savefig('flux_Fnu_r.pdf')
    
    plt.clf()
    plt.plot(rbins_bm,flux_b)
    plt.xlim([0,300])
    
    plt.savefig('flux_b.pdf')
...
#    #Interpolate 2D grid space for grain distribution as a function of size and radial distance values
    ...
    Filter_70 = DustyMM_fdist(4.1,3000,0.05,3.9,120,2,100,140,fFBlue70wr)
#    plt.plot(rbins_bm,MIRI_10,'k')
#    plt.show()
#
#    print(zed)
#
...
#    print(zed)
    ...
    ax.plot(wr,SED_disc, 'b-.', label = 'Disc:'+composition, zorder = 2)
    ...
    for Wavelength in WRange:
        ...
        for column in Fluxvalsg.columns[2:]:  #Extract all flux values except the 'Sum'(last) column - there is no Sum value
        ...
        ax.plot(rbins_bm,10*f_flux(rbins_bm)/np.max(f_flux(rbins_bm)),str(WColour[Ci])+'-',label = str(round(Wavelength,1))+'$\mu$m')
...
#    ax.set_xscale('log')
    ...
    print('Determine Filter Values for MIRI-10 Filter band...')
    ...
    plt.clf()
    fig, ax = plt.subplots(nrows = 1, ncols = 1)
    plt.plot(rbins_bm,MIRI_10,'b:', label = 'JWST/MIRI-1000W')
    plt.plot(rbins_bm,MIRI_21,'r-.', label = 'JWST/MIRI-2100W')
...
#    ax.set_xscale('log')
    ...
    plt.plot(rbins_bm,PACS_70,'b', label = 'Herschel/PACS_70')
    plt.plot(rbins_bm,PACS_100,'g', label = 'Herschel/PACS_100')
    plt.plot(rbins_bm,PACS_160,'r', label = 'Herschel/PACS_160')
