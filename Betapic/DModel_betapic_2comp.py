...
def DustyMM(smin,smax,dfrac,q,rm,rw,rin,rout,composition):
...
#            [r1,th1,ti1] = orbit(a,b,e,P,dtm)
            #r1 = f_BeRad(be,BeIn)
            rb = (2-(1-e**2)/(1 + e*np.cos(thr)))  #probability of finding grain a specific r value
            
##            print(rb)
#
#            print(f'e: {e} / a: {a}')
##            eatest = rb*a
#            eatest = np.multiply.outer(a,rb)
##            print(eatest)
##            print('first array')
##            print(rb*a[0])
##            print('last array')
##            print(rb*a[-1])
###            plt.plot(eatest[0,:])
##            eathist = histogram1d(eatest[50,:], range = [rbins_bins[0],rbins_bins[-1]], bins=len(rbins_bins)-1)
##            plt.plot(eathist)
##
##            probhist = histogram1d(rb*1000, range = [rbins_bins[0],rbins_bins[-1]], bins=len(rbins_bins)-1)
###            plt.plot(probhist*a[1],'k')
###            plt.plot(probhist)
##            plt.show()
##            print(zed)
#
##            dentest = [histogram1d(eatest[i,:], range = [rbins_bins[0],rbins_bins[-1]], bins=len(rbins_bins)-1) for i in np.linspace(0,len(a),len(a)-1)]
#
#            print(dentest)
#
#            print(zed)
            ...
            #binning heliocentric (orbital) distances in binsizes = 1 au and finding "density" or normalised values
#                [denb,bins] = np.histogram(rb*a[i],bins = rbins_bins,density=True)
                denb = histogram1d(rb*a[i], range = [rbins_bins[0],rbins_bins[-1]], bins=len(rbins_bins)-1)  #multiplying
...
def lnlike(theta,phot_wav,phot_flux,phot_unc):
    ...
    func_flux1 = DustyMM(sm,smax_r,dfrac,qv,rm1,rw1,rin,rout,composition)
    #Inner Belt
    composition = comp_inner
    func_flux2 = DustyMM(sm2,smax_r,dfrac2,qv2,rm2,rw2,rin2,rout2, composition)
...
#-----------------------------------------------------------------#
##Scenarios
Scen = 0#0: inner: astro-silicate outer: dirty ice / 1: inner/outer: astro-silicate 2: inner/outer: dirty ice
if Scen == 0:
...
#Inner: AS Outer: DI
sm_outer = [5.04 ,2.11,1.40]
dfrac_outer = [0.48 , 0.07, 0.06]
q_outer = [ 3.68, 0.06,0.05 ]

sm_inner = [ 3.14,1.43 ,1.14 ]
dfrac_inner = [ 0.07, 0.01, 0.01]
q_inner = [ 3.87,0.09, 0.17]

##Inner/Outer: AS/AS
#sm_outer = [2.589,0.85,0.543]
#dfrac_outer = [1.03198, 0.18084, 0.17953]
#q_outer = [ 3.866, 0.05,0.046 ]
#
#sm_inner = [ 3.82,3.04 ,1.47]
#dfrac_inner = [ 0.08296, 0.006, 0.00511]
#q_inner = [ 3.67,0.23, 0.33]

##Inner/Outer: DI/DI
#sm_outer = [5.296,3.36,-2.457]
#dfrac_outer = [0.41668, 0.11784, 0.05683]
#q_outer = [ 3.659, 0.09,0.06 ]
#
#sm_inner = [ 16.71, 2.42,-4.38 ]
#dfrac_inner = [ 0.05207, 0.00343, 0.00251]
#q_inner = [ 3.44,0.33, 0.28]
...
direcsav = '/'+object+'_rm1_'+str(rm1)+'_rm2_'+str(rm2)+'_sm1_'+str(round(sm_outer[0],3))+'_sm2_'+str(round(sm_inner[0],3))+'_inner'+ci+'_outer'+co
...
#cornerfig = object+'_Emcee_corner_nwalkers_'+str(nwalkers)+'_niter'+str(niter)+'_2temp_inner'+ci+'_outer'+co+'.eps'
...
#
#
#print("s_min: {0:1.2f} (+{1:1.2f}, -{2:1.2f}) mu".format(sm_outer[0],sm_outer[1], sm_outer[2]))
#print("dfrac: {0:1.5f} (+{1:1.5f}, -{2:1.5f}) MEarth".format(dfrac_outer[0],dfrac_outer[1], dfrac_outer[2]))
#print("q: {0:1.2f} (+{1:1.2f}, -{2:1.2f}) [-]".format(q_outer[0],q_outer[1], q_outer[2]))
#print("s_min2: {0:1.2f} (+{1:1.2f}, -{2:1.2f}) mu".format(sm_inner[0],sm_inner[1], sm_inner[2]))
#print("dfrac2: {0:1.5f} (+{1:1.5f}, -{2:1.5f}) MEarth".format(dfrac_inner[0],dfrac_inner[1], dfrac_inner[2]))
#print("q2: {0:1.2f} (+{1:1.2f}, -{2:1.2f}) [-]".format(q_inner[0],q_inner[1], q_inner[2]))
#
#
...
#figname = 'SED_'+object+'_disc_para_models_2temp_2comp_combine'+str(niter)+'_'+ci+co+'.eps'
#plt.savefig(figname)
##plt.show()
##print(zed)
#
...
##ax.set_yscale('log')
#ax.legend()
#figname = 'SED_'+object+'_Numberdensity_distance_2temp_normalise_2comp_'+ci+co+'.eps'
#plt.savefig(figname)
