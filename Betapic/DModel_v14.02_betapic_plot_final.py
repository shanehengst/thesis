...
def lnprior(theta):
    ...
    if 1.6 < sm < 10.0 and dfrac < 0.01 and 3.0 < q < 4.0 and 50 < rm < 150 and 0.1 < rw < 40:
        return 0.0
    else:
        return -np.inf
...
#Phot_bpic = np.loadtxt('df_betapic_cold_v2.txt', dtype=[('wavelength', float),('flux', float),('uncertainty', float) ])
...
##Stellar Spectrum##
#HD Beta Pictoris Stellar flux values
#w_Ks = 2.159
#f_Ks = 26.1
#
#fluxstar = ascii.read('ckp00_8250.ascii')
#fluxstar = np.loadtxt('ckp00_8250.ascii', dtype=[('wavelength', float), ('flux', float)])
#wa = fluxstar["wavelength"]/10000
#fnu = (fluxstar["flux"])*wa**2/c
#
#fnui = interpolate.interp1d(wa,fnu, kind = 'linear')
#f_Ksi = fnui(w_Ks)
#fnu = (fnu/f_Ksi)*f_Ks
...
#far infrared
fir_lam = [ 60, 70, 100.0,  160.0 ]
fir_flx = [18.5, 16, 9.8,    5.1]
fir_unc = [3.7, 0.8, 1.058, 0.5]

#sub-mm/mm
smm_lam = [250.0,  350.0, 500.0,  850.0,1200]
smm_flx = [1.9,   0.72,   0.38, 0.058,0.036]
smm_unc = [0.1,  0.05,  0.03,  0.006,0.01]
...
#Fast function
[SED_total1,SED_cdisc,Mperbc] = DustyMM(sm,smax,dfrac,qv,rm,rw,rin,rout)
#[SED_total2,SED_wdisc,Mperbw] = DustyMM(sm,smax,dfrac,qv,rm,rw,rin,rout)
...
#rin = 1
#rout = 15
#in. Grain Size: 6.1032784606213415, dfrac: 0.47119839133440594, q: 3.626953382208915, rmean: 98.8110980799559, rsigma: 58.75543275287188
#Min. Grain Size: 9.307712472419816, dfrac: 0.1086601323108634, q: 3.1745897474577585, rmean: 6.593152557919095, rsigma: 1.31742357341675#inner belt
sm = 9.00
smax = 3000
dfrac = 0.112540
rm = 9.06
...
#
#Slow function
#[df_rbins_p2, df_rbins_pb2, df_rbins_dN2, df_rbins_dNb2, df_rbins_dM2,df_rbins_dMb2, df_fluxsm2, df_fluxsmb2, y_values_nb2, y_values_b2, grainsizes2,gr_blowout2,MPerb2,SED_total22,SED_total_nb22,SED_disc22,SED_disc_nb22] = BetaGrains(sm,smax,dfrac,qv,rm,rw,rin,rout,rho,100,hd_bs,Tg_sr,Qpr_s,Qsca_sw,Qabs_sw,wr)

#                    print(np.max(SED_disc2))

#Fraction of disc mass
dfracf = dfrac*Mperbw/100

SED_belts = np.add(SED_cdisc,SED_wdisc)
SED_stardisc = np.add(flux_nu,SED_belts)
#
#SED_belts_slow = np.add(SED_disc2,SED_disc22)
#SED_stardisc2 = np.add(SED_belts_slow,flux_nu)

c2 = Chi2(lam,flx,unc,wr ,SED_stardisc)
#c22 = Chi2(lam,flx,unc,wr, SED_stardisc2)
#chi2.append(c2)
#print(f'Chi2(fast): {c2} / Chi2(slow): {c22}  ')
...
direcsav = '/'+object+'_smin'+str(round(sm,2))+'_rm'+str(rm)+'_dfrac'+str(dfrac)+'_q'+str(qv)+'_rw'+str(rw)+'_rin'+str(rin)+'_rout'+str(rout)+'_ch2'+str(np.round(c2,2))
...
#plt.plot(lam,flx,'o')
#plt.plot(wr,SED_stardisc,'k', label = 'Star+belts+fast', zorder = 3)
plt.plot(wr,SED_stardisc2, 'y', label = 'Star+betls+slow', zorder = 6)
plt.plot(wr,SED_wdisc, 'r:', label = 'Warm Belt', zorder = 1)
plt.plot(wr,SED_cdisc, 'c:', label = 'Cold Belt', zorder = 2)
#plt.plot(wr,SED_disc22, 'r:', label = 'Warm Belt', zorder = 4)
#plt.plot(wr,SED_disc2, 'c:', label = 'Cold Belt', zorder = 5)
...
#ax.plot(wr,flux_nu, 'k:', label = 'Star')
...
figname = 'SED_'+object+'_disc_para_models_2temp_2comp.eps'
#plt.savefig(figname)
plt.show()
print(zed)
...
plt.clf()
fig, ax = plt.subplots(nrows = 1, ncols = 1)
ax.plot(rbins_bm,df_rbins_dNb['Sum']/np.max(df_rbins_dNb['Sum']),'b-.',label = 'Cool Beta disc')
ax.plot(rbins_bm,df_rbins_dN['Sum']/np.max(df_rbins_dN['Sum']), 'b', label = 'Cool non-Beta disc')
ax.plot(rbins_bm,df_rbins_dNb2['Sum']/np.max(df_rbins_dNb2['Sum']),'r-.',label = 'Warm Beta disc')
ax.plot(rbins_bm,df_rbins_dN2['Sum']/np.max(df_rbins_dNb2['Sum']), 'r', label = 'Warm non-Beta disc')
ax.set_xlim([0,300])
...
#ax.set_yscale('log')
ax.legend()
figname = 'SED_'+object+'_Numberdensity_distance_2temp_normalise_2comp.eps'
plt.savefig(figname)
...
#        bgf.append(val)                             #append Flux value
        ...
        elif float(bvalues[0]) > ming and float(bvalues[0]) < midg:
...
#    ax.plot(df_fldis['Distance'],4*df_fldis['Flux']/np.max(df_fldis['Flux']), label = 'All sizes')
    ax.plot(df_fldis['Distance'],4*df_fldis['Flux']/(df_fldis.iloc[0, df_fldis.columns.get_loc('Flux')]), label = 'All sizes')
   

    df_fldis_s = pd.DataFrame({'Distance': bgd_s, 'Flux': bgf_s})
    df_fldis_s = df_fldis_s.groupby('Distance', as_index=False).sum()
    fl_dis_s = df_fldis_s['Flux'].to_numpy()
#fl_s_a = fl_dis_s[0]
#    print(fl_dis_s)
#    print(df_fldis_s['Flux'])
#    print(zed)
#    ax.plot(df_fldis_s['Distance'],3*df_fldis_s['Flux']/np.max(df_fldis_s['Flux']), label = 's <= 1')

    ax.plot(df_fldis_s['Distance'],3*df_fldis_s['Flux']/(df_fldis_s.iloc[0, df_fldis_s.columns.get_loc('Flux')]), label = labels)
#    ax.plot(df_fldis_s['Distance'],3*df_fldis_s['Flux']/fl_s_a, label = 's <= 1')
   

    df_fldis_m = pd.DataFrame({'Distance': bgd_m, 'Flux': bgf_m})
    df_fldis_m = df_fldis_m.groupby('Distance', as_index=False).sum()
#    fl_dis_m = df_fldis_m['Flux'].to_numpy()
#    fl_m_a = fl_dis_m[0]
#    ax.plot(df_fldis_m['Distance'],2*df_fldis_m['Flux']/np.max(df_fldis_m['Flux']), label = '1 < s < 10')
   
    ax.plot(df_fldis_m['Distance'],2*df_fldis_m['Flux']/(df_fldis_m.iloc[0, df_fldis_m.columns.get_loc('Flux')]), label = labelm)
#    ax.plot(df_fldis_m['Distance'],2*df_fldis_m['Flux']/fl_m_a, label = '1 < s < 10')


    df_fldis_b = pd.DataFrame({'Distance': bgd_b, 'Flux': bgf_b})
    df_fldis_b = df_fldis_b.groupby('Distance', as_index=False).sum()
#    fl_dis_b = df_fldis_b['Flux'].to_numpy()
#    fl_b_a = fl_dis_b[0]
#    ax.plot(df_fldis_b['Distance'],df_fldis_b['Flux']/np.max(df_fldis_b['Flux']), label = 's >= 10')
    ax.plot(df_fldis_b['Distance'],df_fldis_b['Flux']/(df_fldis_b.iloc[0, df_fldis_b.columns.get_loc('Flux')]), label = labelb)
#    ax.plot(df_fldis_b['Distance'],df_fldis_b['Flux']/fl_b_a, label = 's >= 10')
